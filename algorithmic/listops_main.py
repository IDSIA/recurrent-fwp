...
from listops_data import ListOpsDataset
from model import (
    LSTMModel, TrafoModel, LinearTransformer, DeltaNetModel,
    FastRNNModel, FastFFslowRNNModel, RecDeltaNetModel, OwnTransformer)
from eval_utils import listops_compute_accuracy
...
parser.add_argument('--level', type=str, default="a10d10max201min19",
...
pad_idx = 0  # src_pad does not matter; as padding is aligned.
...
train_file = f"{data_path}/train_{args.level}.txt"
valid_file = f"{data_path}/valid_{args.level}.txt"
test_file = f"{data_path}/test_{args.level}.txt"

# Construct dataset
train_data = ListOpsDataset(train_file, pad_idx=pad_idx)
vocab = train_data.vocab
valid_data = ListOpsDataset(valid_file, pad_idx=pad_idx, vocab=vocab)
...
# Common params:
in_vocab_size = vocab.size()
out_vocab_size = 10  # change
...
for ep in range(num_epoch):
    for idx, batch in enumerate(train_data_loader):
        ...
        logits = model(inputs)
        # select only the last position
        seq_len = seq_len.unsqueeze(1).unsqueeze(2).expand(
            -1, -1, logits.size(2))
        logits = logits.gather(1, seq_len)
    ...
    with torch.no_grad():
        ...
        v_loss, v_acc = listops_compute_accuracy(
            model, valid_data_loader, loss_fn, show_example=False)
...
if best_epoch > 1:  # load the best model and evaluate on the test set
    ...
    test_data = ListOpsDataset(test_file, pad_idx=pad_idx, vocab=vocab)
    ...
    with torch.no_grad():
        test_loss, test_acc = listops_compute_accuracy(
            model, test_data_loader, loss_fn, show_example=False)
